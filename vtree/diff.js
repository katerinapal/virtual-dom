...
function walk(a, b, patch, index) {
    ...
    if ((0, _isThunk.isThunk)(a) || (0, _isThunk.isThunk)(b)) {
        ...
        apply = appendPatch(apply, new _vpatch.VirtualPatch(_vpatch.VirtualPatch.REMOVE, a, b));
        ...
        if ((0, _isVnode.isVirtualNode)(a)) {
            if (a.tagName === b.tagName && a.namespace === b.namespace && a.key === b.key) {
                ...
                if (propsPatch) {
                    apply = appendPatch(apply, new _vpatch.VirtualPatch(_vpatch.VirtualPatch.PROPS, a, propsPatch));
                }
                apply = diffChildren(a, b, patch, apply, index);
            } else {
                apply = appendPatch(apply, new _vpatch.VirtualPatch(_vpatch.VirtualPatch.VNODE, a, b));
                applyClear = true;
            }
        } else {
            apply = appendPatch(apply, new _vpatch.VirtualPatch(_vpatch.VirtualPatch.VNODE, a, b));
        ...
        if (!(0, _isVtext.isVirtualText)(a)) {
            apply = appendPatch(apply, new _vpatch.VirtualPatch(_vpatch.VirtualPatch.VTEXT, a, b));
            applyClear = true;
        ...
        apply = appendPatch(apply, new _vpatch.VirtualPatch(_vpatch.VirtualPatch.WIDGET, a, b));
    }
...
function diffChildren(a, b, patch, apply, index) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        if (!leftNode) {
            if (rightNode) {
                ...
                apply = appendPatch(apply, new _vpatch.VirtualPatch(_vpatch.VirtualPatch.INSERT, null, rightNode));
            }
        } else {
    ...
    if (orderedSet.moves) {
        // Reorder nodes last
        apply = appendPatch(apply, new _vpatch.VirtualPatch(_vpatch.VirtualPatch.ORDER, a, orderedSet.moves));
    }
...
function destroyWidgets(vNode, patch, index) {
    if ((0, _isWidget.isWidget)(vNode)) {
        if (typeof vNode.destroy === "function") {
            patch[index] = appendPatch(patch[index], new _vpatch.VirtualPatch(_vpatch.VirtualPatch.REMOVE, vNode, null));
        }
...
function thunks(a, b, patch, index) {
    ...
    if (hasPatches(thunkPatch)) {
        patch[index] = new _vpatch.VirtualPatch(_vpatch.VirtualPatch.THUNK, null, thunkPatch);
    }
}
...
function unhook(vNode, patch, index) {
    if ((0, _isVnode.isVirtualNode)(vNode)) {
        if (vNode.hooks) {
            patch[index] = appendPatch(patch[index], new _vpatch.VirtualPatch(_vpatch.VirtualPatch.PROPS, vNode, undefinedKeys(vNode.hooks)));
        }
